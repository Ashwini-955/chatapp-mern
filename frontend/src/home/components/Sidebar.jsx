    ...
    <div className="h-screen w-72 bg-white/90 backdrop-blur-xl border-r border-gray-200 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1">
        {searchUser.length > 0 ? (
          searchUser.map((user) => (
            ...
            >
              <img
                ...
                src={userAuth?.profilepic || "https://ui-avatars.com/api/?name=${encodeURIComponent(user.fullname)}&background=0D8ABC&color=fff&size=100"}
                alt={user.fullname}
          ...
          <p className="text-gray-500 text-center py-4">Search for users to start chatting</p>
        )}
      </div>
    </div>
  );
